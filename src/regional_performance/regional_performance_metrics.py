...
def get_order_vol_by_state_at_customer_level(conn, sql_base_path:Path) -> pd.DataFrame:
    ...
    return execute_query(conn=conn,
                         ...
                         expected_columns=['customer_unique_id', 'total_orders', 'state'])




# -----------------------------------------------------------------------------------
# 3. Get the total number of orders and a sum ot total revenue by month per state:
# -----------------------------------------------------------------------------------

def get_orders_and_revenue_by_time_period_per_state(conn, sql_base_path:Path) -> pd.DataFrame:
    """
    This function will be responsible to count the total number of orders and a total sum of revenue per month for each state

    Returns:
    --------
    pd.Dataframe with time period which represents the months, the abbreviated name for the state, a total number of orders and a total
    amount of revenue generated by each state per month.
    """

    return execute_query(conn=conn,
                         sql_path=sql_base_path / "baseline_performance.sql",
                         query_name= "orders_and_revenue_by_time_period_per_state",
                         expected_columns=['time_period', 'state', 'total_orders', 'total_revenue'])